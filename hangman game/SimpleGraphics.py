...
def MakeWindow(Mx,My,tickinterval_x=1,tickinterval_y=1,labels=True,bgcolor=WHITE):
    """
    Creates a window with x range x<=Mx and y range y<=My

    If labels is set to False, it will turn off the labeled axes.
    Labeling will not look good if Mx or My is too large, e.g., Mx>10.
    tickinterval_x and tickinterval_y represent the frequency of adding labels on x and y 
    axis, repectively. e.g., tickinterval_x=5 indicates to add a label on x axis 
    at an interval of 5 units.
    The window will have a background color specified by bgcolor.
    The default is white.
    
    Preconditions: M is positive number, tickinterval_x is positive number,tickinterval_y is 	
    positive number, labels is boolean,and bgcolor is an rgb array.
    
    Usage:
       MakeWindow(5,6,tickinterval_x=5)
       MakeWindow(5,10,labels=False)
       MakeWindow(5,5,bgcolor=PURPLE)
       MakeWindow(5,10,labels=False,bgcolor=[0.87 1.00 0.00])
    """

    plt.figure(figsize=(8,8), dpi=80)
    # Where to put the axis ticks.
    plt.xticks(np.arange(0,Mx,tickinterval_x))
    plt.yticks(np.arange(0,Mx,tickinterval_y))
